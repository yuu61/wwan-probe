...
`-Nmea` は `-Gps` を含みます。起動時に一度だけ UAC で NMEA 受信ヘルパーを昇格させ、TUI の `s` で衛星一覧と通常画面を切り替えます。
...
**他のNMEA診断ツールとは同時に実行しないでください。** `lte_monitor.ps1 -Nmea` とはロックを共有し、同時には実行できません。
ネイティブ GNSS の操作と NMEA の解析はモニターと共通です (`src/infrastructure/GnssDevice.ps1`, `Nmea.ps1`)。
