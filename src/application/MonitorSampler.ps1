...
function Start-MonitorSample {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseShouldProcessForStateChangingFunctions', '')]
    param($Sampler, $Modem, $At, $GpsReceiver, $NmeaReceiver)
    ...
    $null = $Sampler.Pipeline.AddCommand('Get-LteSnapshot').AddArgument($Modem).AddArgument($At).AddArgument($GpsReceiver).AddArgument($NmeaReceiver)
